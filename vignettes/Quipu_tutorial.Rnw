...
\subsection{New formats and features}

The raw data format parameter accepts a new format based on a simple scoring format. See the example file:

<<newformat>>=
datFile = system.file("sample_data/simdat.xlsx", package = "quipu")

dat = quipu::read_scores(datFile)

file.copy(datFile, getwd())
if(interactive()){
  quipu::quipu_start()
}
@
